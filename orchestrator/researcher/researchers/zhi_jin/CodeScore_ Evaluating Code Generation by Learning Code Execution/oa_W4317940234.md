...
<table><tr><td>def bubbleSort(arr): Reference Code (a)
n = len(arr)
for i in range(n):
    for j in range(0, n-i-1):
        if arr[j] &gt; arr[j+1]:
            arr[j], arr[j+1] = arr[j+1], arr[j]</td><td>▪ (a) bubbleSort([5,3,2,1,4]) → [1,2,3,4,5]</td></tr><tr><td>def bubbleSort(arr): Generated Code (b)
n = len(arr)
for i in range(n):
    for j in range(0, n-i-1):
        if arr[j] = arr[j+1]:
            arr[j], arr[j+1] = arr[j+1], arr[j]</td><td>▪ (b) bubbleSort([5,3,2,1,4]) → error
▪ BLEU(a,b) = 0.961
▪ CodeBLEU(a,b) = 0.884</td></tr><tr><td>def sortBubble (Nums): Generated Code (c)
num_len = len(Nums)
for j in range(num_len):
    ...
    for i in range(num_len - 1 - j):
        ...
        sign = True
    if not sign:
        break</td><td>▪ (c) sortBubble([5,3,2,1,4]) → [1,2,3,4,5]
▪ BLEU(a,c) = 0.204
▪ CodeBLEU(a,c) = 0.265</td></tr></table>
...
Match-based CEMs. Besides these commonly used BLEU [39], Accuracy, and CodeBLEU [48], some niche CEMs [41] are also applied to code evaluation, e.g., METEOR [3], ROUGE [30], and CrystalBLEU [12]. However, these aforementioned match-based CEMs merely measure the surfacelevel differences in code and do not take into account the functional correctness of the generated code.
...
![](images/eb88e5b7db501584eb331980ed68435664d4a0222124f0241b6e91cc236a5214.jpg)

Generated code (g)

![](images/d2c397306cb7245f74deee229762640c013ea8a02da8780809c10c0c6f86dfdd.jpg)

Reference code (r)

![](images/5a8c9d58ab78fc55c5d5feb260f84cb18ee24bb08696a3bddc85c6f1e5428b25.jpg)
...
$$
\text { PassRatio } = \frac {1}{| C _ {p} |} \sum_ {c \in C _ {p}} \mathbb {I} \left\{\operatorname{Eval} \left(\mathbf {g} _ {p}, \mathcal {I} _ {p, c}\right) = O _ {p, c} \right\}. \tag {1}
$$

where | · | indicates the element number of a set, I {·} is an indicator function, which outputs 1 if the condition is true and 0 otherwise, and Eval $\left( \mathbf { g } _ { p } , \mathcal { I } _ { p , c } \right)$ represents an evaluation function that obtains outputs of code $\mathbf { g } _ { p }$ by way of executing it with $\mathcal { T } _ { p , c }$ as input.
...
$$
\text { Executability } = \left\{ \begin{array}{l} 1, \text {   if   code   is   executable }, \\ 0, \text {   otherwise }. \end{array} \right. \tag {2}
$$
...
![](images/50ff479994e34704b5a741201ce0d2cdb6b55bab6b2d3ab25bed700935a00caa.jpg)
...
where $\mathcal { L } _ { C }$ focuses on predicting PassRatio, and $\mathcal { L } _ { E }$ on predicting code execution correctness. $\mathcal { L } _ { C }$ and $\mathcal { L } _ { E }$ are defined as:

$$
\mathcal {L} _ {C} = \left(\text { CodeScore } - \text { PassRatio }\right) ^ {2}, \tag {5}
$$

$$
\mathcal {L} _ {E} = - \log \mathrm{p} (\text { Exec } \mid \text { Executability }), \tag {6}
$$

where $\mathcal { L } _ { C }$ measures the squared difference between the predicted CodeScore and the actual Pass-Ratio. $\mathcal { L } _ { E }$ represents the negative log of the conditional probability of Exec given its Executability. The conditional probability is modeled as:

$$
\mathbf {p} (\text { Exec } \mid \text { Executability }) = \left\{ \begin{array}{l l} \mathbf {p} (\text { Exec }), & \text { if   Executability } = 1, \\ 1 - \mathbf {p} (\text { Exec }), & \text { otherwise }, \end{array} \right. \tag {7}
$$
...
3.2.1 Pooling Layer. For LLMs, the pooling layer plays a critical role in enhancing the model’s ability to capture and utilize information more effectively. The work [46, 55, 63] shows that exploiting information from different layers of LLM generally results in superior performance than only the last layer. Therefore, following the work [40], we pool information from different layers by using a layer-wise attention mechanism and the final embedding of a token ?? can be computed as:

$$
e _ {t} = \gamma \sum_ {k = 1} ^ {l} e _ {t} ^ {k} h ^ {k}, \tag {8}
$$

where ?? indicates the number of layers, and ?? and $h ^ { k }$ are trainable weights.

3.2.2 Unified Embedding. We require an efficient and comprehensive representation to encapsulate the unified input sequence ??. Generally, there are two standard methods to extract the representation of ??, i.e., averaging all token embeddings and using the first token embedding. While the first method is straightforward and includes information from all tokens, it may dilute the significance of more critical tokens and introduce extraneous noise. The first token of our base models is specifically designed to be a summary token6. Moreover, the work [42, 56] also proves the superiority of using the first token embedding compared to averaging all token embeddings in various applications. Thus, we employ the final embedding of the first token $e _ { f i r s t }$ as the representation of the unified input sequence ??.

3.2.3 Unified training. In UniCE, $e _ { f i r s t }$ is fed to a feed-forward neural network to output a score and/or a category. To unify three evaluation input formats into UniCE, we apply multi-task learning for training. Specifically, for each step, we assign three sub-steps for three input formats, yielding $\mathcal { L } ^ { R e f } , \mathcal { L } ^ { N \tilde { L } }$ , and $\mathcal { L } ^ { R e f + \dot { N } L }$ , respectively. A Ref&NL data can be regarded as three input format data to yield three losses, while Ref-only and NL-only data can only compute the corresponding $\mathcal { L } ^ { R e f }$ and $\mathcal { L } ^ { N L }$ . The final learning objective of UniCE is to minimize $\mathcal { L } ^ { U n i }$ :

$$
\mathcal {L} ^ {U n i} = \mathcal {L} ^ {R e f} + \mathcal {L} ^ {N L} + \mathcal {L} ^ {R e f + N L}, \tag {9}
$$

where $\mathcal { L } ^ { R e f } , \mathcal { L } ^ { N L }$ , and $\mathcal { L } ^ { R e f + N L }$ are compute via Eq. 4 using corresponding format data as input.
...
• RQ3: What is the contribution of $L ^ { U n i }$ to UniCE for three input formats, compared to their respective losses?   
...
$$
\mathrm{BLEU} = B P \cdot \exp \left(\sum_ {m = 1} ^ {n} \omega_ {m} \log p _ {m}\right),
$$
...
where ???? represents the penalty item, $l _ { g }$ represents the length of generated code, $l _ { r }$ represents the length of reference code, and $\omega _ { m }$ and $\textstyle { p _ { m } }$ represents the weighted coefficient and precision of ??-gram, respectively.
...
$$
\text { CodeBLEU } = \alpha \cdot \text { BLEU } + \beta \cdot \text { BLEU } _ {\text { weight }}
$$

$$
+ \delta \cdot \operatorname{Match} _ {a s t} + \zeta \cdot \operatorname{Match} _ {d f},
$$

where $\alpha , \beta , \delta$ and $\zeta$ are weights (usually set to 0.25, as well as in this paper), $\mathrm { B L E U } _ { w e i g h t }$ is a weighted BLEU with different weights for various tokens, $\mathbf { M a t c h } _ { a s t }$ is syntactic AST matching, which explores the syntactic information of the code, and $\operatorname { M a t c h } _ { d f }$ is semantic dataflow matching, which considers the semantic similarity between generated code and reference code.
...
$$
R _ {\text {BERT}} = \frac {1}{| \mathbf {x} |} \sum_ {\mathbf {x} _ {i} \in \mathbf {x}} \max _ {\hat {\mathbf {x}} _ {j} \in \hat {\mathbf {x}}} \mathbf {x} _ {i} ^ {\top} \hat {\mathbf {x}} _ {j}, \quad P _ {\text {BERT}} = \frac {1}{| \hat {\mathbf {x}} |} \sum_ {\hat {\mathbf {x}} _ {j} \in \hat {\mathbf {x}}} \max _ {\mathbf {x} _ {i} \in \mathbf {x}} \mathbf {x} _ {i} ^ {\top} \hat {\mathbf {x}} _ {j},
$$

$$
F _ {\mathrm{BERT}} = 2 \frac {P _ {\mathrm{BERT}} \cdot R _ {\mathrm{BERT}}}{P _ {\mathrm{BERT}} + R _ {\mathrm{BERT}}}.
$$
...
• AvgPassRatio [18] is defined as the average proportion of test cases that generated codes $\mathbf { g } _ { p } ^ { \prime } s$ pass:

$$
\text { AvgPassRatio } = \frac {1}{| P |} \sum_ {p \in P} \frac {1}{| C _ {p} |} \sum_ {c \in C _ {p}} \mathbb {I} \left\{\operatorname{Eval} \left(\mathbf {g} _ {p}, \mathcal {I} _ {p, c}\right) = \mathcal {O} _ {p, c} \right\}, \tag {10}
$$

where | · | indicates the element number of a set, I(·) is an indicator function, which outputs 1 if the condition is true and 0 otherwise, and Eval $\left( \mathbf { g } _ { p } , \mathcal { I } _ { p , c } \right)$ represents an evaluation function that obtains outputs of code $\mathbf { g } _ { p }$ by way of executing it with $\mathcal { T } _ { p , c }$ as input.

As mentioned above, continuous PassRatio (the item of AvgPassRatio) can better reflect the execution similarity of different codes than binary Passability (the item of Pass@1 8). Therefore, in this paper, we mainly compare the correlation between CodeScore and AvgPassRatio in Executionbased CEMs.

The input format of the proceeding baselines is Ref-only and each of them except COMET is in the range of 0 to 1.

4.1.3 Correlation Evaluation. We use three major correlation coefficients in statistics (i.e., Kendall-Tau(??), Spearman R $\left( r _ { s } \right)$ , and Pearson R $( r _ { p } )$ to evaluate the correlation between each EM and functional correctness. Furthermore, we use Mean Absolute Error (MAE) to assess the absolute error between them.

• Kendall-Tau (??) [23] is a statistic used to measure the ordinal association between two measured data:

$$
\tau = \frac {\text { Concordant } - \text { Discordant }}{\text { Concordant } + \text { Discordant }}, \tag {11}
$$

where ???????????????????? indicates the number of occurrences that two evaluation data $M ^ { 1 }$ and $M ^ { 2 }$ exist either both $M _ { i } ^ { 1 } > M _ { j } ^ { 1 }$ and $M _ { i } ^ { 2 } > M _ { j } ^ { 2 }$ or both $M _ { i } ^ { 1 } < M _ { j } ^ { 1 }$ and $M _ { i } ^ { 2 } < M _ { j } ^ { 2 }$ , and ???????????????????? indicates the number of occurrences opposite to ????????????????????.

• Spearman R $\left( \mathbf { r } _ { s } \right)$ [34] is a nonparametric measure of rank correlation (statistical dependence between the rankings of two data):

$$
r _ {s} = \frac {\operatorname{cov} (\mathrm{R} (M ^ {1}) , \mathrm{R} (M ^ {2}))}{\sigma_ {\mathrm{R} (M ^ {1})} \sigma_ {\mathrm{R} (M ^ {2})}}, \tag {12}
$$

where $\mathbb { R } ( M ^ { 1 } )$ and $\mathbb { R } ( M ^ { 2 } )$ represent the rankings of $M ^ { 1 }$ and $M ^ { 2 }$ , cov(·, ·) means the covariance function, and $\sigma _ { M }$ means the standard deviation of ??.

• Pearson R $( \mathbf { r _ { p } } )$ [4] is a measure of linear correlation between two data:

$$
r _ {s} = \frac {\operatorname{cov} (M ^ {1} , M ^ {2})}{\sigma_ {M ^ {1}} \sigma_ {M ^ {2}}}. \tag {13}
$$

Table 3. Correlation comparison of functional correctness on APPS-Eval dataset. 

<table><tr><td>Method</td><td>Value</td><td> $\tau \uparrow$ </td><td> $r_s \uparrow$ </td><td> $r_p \uparrow$ </td><td>MAE ↓</td><td>Execution Time ↓</td></tr><tr><td colspan="7">Match-based CEM</td></tr><tr><td>BLEU [39]</td><td>0.0094</td><td>0.1055</td><td>0.1156</td><td>0.0959</td><td>0.1164</td><td>1.0 × (26.0s)</td></tr><tr><td>Accuracy</td><td>0.0001</td><td>0.0079</td><td>0.0095</td><td>0.0196</td><td>-</td><td>0.1 ×</td></tr><tr><td>CodeBLEU [48]</td><td>0.2337</td><td>0.1035</td><td>0.1533</td><td>0.1085</td><td>0.2005</td><td>7.8 ×</td></tr><tr><td>CrystalBLEU [12]</td><td>0.0242</td><td>0.0906</td><td>0.1347</td><td>0.0887</td><td>0.1709</td><td>0.3 ×</td></tr><tr><td colspan="7">LLM-based EM</td></tr><tr><td>BERTScore [63]</td><td>0.8629</td><td>0.0916</td><td>0.1375</td><td>0.0718</td><td>0.6874</td><td>56.7 ×</td></tr><tr><td>COMET [46]</td><td>0.0165</td><td>0.0904</td><td>0.1126</td><td>0.1187</td><td>0.1751</td><td>84.0 ×</td></tr><tr><td>CodeBERTScore [67]</td><td>0.7583</td><td>0.1219</td><td>0.1801</td><td>0.1323</td><td>0.5885</td><td>27.8 ×</td></tr><tr><td colspan="7">CodeScore</td></tr><tr><td colspan="7">Ref-only (g + r)</td></tr><tr><td>UniCE with  $\mathcal{L}^{Ref}$ </td><td>0.1996</td><td>0.4760</td><td>0.6473</td><td>0.6620</td><td>0.1202</td><td rowspan="2">33.7 ×</td></tr><tr><td>UniCE with  $\mathcal{L}^{Uni}$ </td><td>0.1977</td><td>0.5033</td><td>0.6693</td><td>0.6929</td><td>0.1128</td></tr><tr><td colspan="7">NL-only (g + n)</td></tr><tr><td>UniCE with  $\mathcal{L}^{NL}$ </td><td>0.2035</td><td>0.4679</td><td>0.6359</td><td>0.6855</td><td>0.1189</td><td rowspan="2">37.9 ×</td></tr><tr><td>UniCE with  $\mathcal{L}^{Uni}$ </td><td>0.2016</td><td>0.4901</td><td>0.6486</td><td>0.6905</td><td>0.1120</td></tr><tr><td colspan="7">Ref&amp;NL (g + r + n)</td></tr><tr><td>UniCE with  $\mathcal{L}^{Ref+NL}$ </td><td>0.1837</td><td>0.3865</td><td>0.5419</td><td>0.6152</td><td>0.1274</td><td rowspan="2">44.2 ×</td></tr><tr><td>UniCE with  $\mathcal{L}^{Uni}$ </td><td>0.1820</td><td>0.5275 (↑ 40.56%)</td><td>0.7040 (↑ 55.07%)</td><td>0.7210 (↑ 58.87%)</td><td>0.1044</td></tr><tr><td colspan="7">Execution-based CEM</td></tr><tr><td>13 test cases per task</td><td>0.0978</td><td>0.3360</td><td>0.4108</td><td>0.4987</td><td>0.1327</td><td>1.5k ×</td></tr><tr><td>181 test cases per task</td><td>0.1790</td><td>-</td><td>-</td><td>-</td><td>-</td><td>20.7k ×</td></tr></table>

• Mean Absolute Error (MAE) is a measure of errors between paired data:

$$
\mathrm{MAE} = \frac {\sum_ {i = 1} ^ {N} \left| M _ {i} ^ {1} - M _ {i} ^ {2} \right|}{N}, \tag {14}
$$

where | · | means the absolute-value function.

4.1.4 Implementation Details. In this paper, UniXcoder [15] is employed as the base LLM of UniCE, which has the similar parameter size of LLMs in BERTScore [63] and COMET [46], and larger LLMs can usually lead to better results. We format the input sequences as “[CLS] ?? [SEP] ?? [SEP] ?? [SEP]”, where [CLS] and [SEP] are the special tokens in vocabulary, and we replace ??, ??, and ?? with the generated code, reference code, and NL description, respectively. For the balance of three input formats during the training process, we first sample an NL along with its corresponding generated code and reference code. They are then employed to construct data in three formats: Ref-only, NL-only, and Ref&NL. Finally, these formats are combined for training UniCE. In all experiments of this paper, we train UniCE on the train set of APPS-Eval. We fine-tune UniCE on the train set of MBPP-Eval only when we specially mention it in our paper. We train UniCE with Adam [24] optimizer on a single GPU of Tesla A100-PCIe-40G. Empirically, the learning rate is set to 0.001 and the training epoch is set to 5. The feedforward neural network of UniCE consists of 3 linear transitions with the hyperbolic tangent (Tanh) activation functions, where the corresponding output dimensions are 3,072, 1,024, and 2, respectively. The input token length is limited to 1024. To mitigate the instability of model training, we exhibit the average performance of UniCE running five times.

Table 4. Correlation comparison of functional correctness on MBPP-Eval and HE-Eval datasets. 

<table><tr><td rowspan="2">Method</td><td colspan="3">MBPP-Eval</td><td colspan="3">HE-Eval</td></tr><tr><td>Value</td><td> $r_s \uparrow$ </td><td>Execution Time ↓</td><td>Value</td><td> $r_s \uparrow$ </td><td>Execution Time ↓</td></tr><tr><td colspan="7">Match-based CEM</td></tr><tr><td>BLEU [39]</td><td>0.1186</td><td>0.1784</td><td>1.0 × (0.87s)</td><td>0.2436</td><td>0.0987</td><td>1.0 × (1.96s)</td></tr><tr><td>Accuracy</td><td>0.0004</td><td>0.0299</td><td>0.1 ×</td><td>0.0011</td><td>0.0456</td><td>0.1 ×</td></tr><tr><td>CodeBLEU [13]</td><td>0.1827</td><td>0.2902</td><td>5.0 ×</td><td>0.3452</td><td>0.3308</td><td>6.3 ×</td></tr><tr><td>CrystalBLEU [12]</td><td>0.0295</td><td>0.1645</td><td>0.3 ×</td><td>0.0427</td><td>0.2171</td><td>0.4 ×</td></tr><tr><td colspan="7">LLM-based EM</td></tr><tr><td>BERTScore [63]</td><td>0.8842</td><td>0.1522</td><td>62.0 ×</td><td>0.9008</td><td>0.1214</td><td>57.5×</td></tr><tr><td>COMET [46]</td><td>-0.5001</td><td>0.2681</td><td>69.0 ×</td><td>0.0879</td><td>0.1437</td><td>58.2×</td></tr><tr><td>CodeBERTScore [67]</td><td>0.7863</td><td>0.2490</td><td>44.9 ×</td><td>0.8091</td><td>0.3196</td><td>47.4 ×</td></tr><tr><td colspan="7">CodeScore</td></tr><tr><td colspan="7">Ref-only (g + r)</td></tr><tr><td>UniCE with  $\mathcal{L}^{Ref}$ </td><td>0.2975</td><td>0.5864</td><td rowspan="2">17.2 ×</td><td>0.3426</td><td>0.5671</td><td rowspan="2">30.2×</td></tr><tr><td>UniCE with  $\mathcal{L}^{Uni}$ </td><td>0.3253</td><td>0.5999</td><td>0.4257</td><td>0.6378</td></tr><tr><td colspan="7">NL-only (g + n)</td></tr><tr><td>UniCE with  $\mathcal{L}^{NL}$ </td><td>0.3364</td><td>0.4492</td><td rowspan="2">12.6 ×</td><td>0.4985</td><td>0.5634</td><td rowspan="2">30.6×</td></tr><tr><td>UniCE with  $\mathcal{L}^{Uni}$ </td><td>0.3327</td><td>0.5719</td><td>0.5624</td><td>0.6215</td></tr><tr><td colspan="7">Ref&amp;NL (g + r + n)</td></tr><tr><td>UniCE with  $\mathcal{L}^{Ref+NL}$ </td><td>0.2905</td><td>0.5926</td><td rowspan="2">20.7 ×</td><td>0.4059</td><td>0.5965</td><td rowspan="2">32.9×</td></tr><tr><td>UniCE with  $\mathcal{L}^{Uni}$ </td><td>0.3247</td><td>0.6027 (↑ 31.25%)</td><td>0.4731</td><td>0.6597 (↑ 32.89%)</td></tr><tr><td colspan="7">Execution-based CEM</td></tr><tr><td>8 test cases per task</td><td>0.2670</td><td>0.6826</td><td>1.0k ×</td><td>0.5994</td><td>0.6981</td><td>1.9k ×</td></tr><tr><td>108 test cases per task</td><td>0.2890</td><td>-</td><td>28.7k ×</td><td>0.3695</td><td>-</td><td>21.7k ×</td></tr></table>

# 4.2 Experimental Results

4.2.1 RQ1: Effect of CodeScore. As illustrated in Table 3, CodeScore exhibits a significantly stronger correlation with functional correctness than existing match-based CEMs and LLM-based EMs, which display weak or extremely weak correlations with Ground Truth on APPS-Eval. Compared with the top-performing EM among other EMs, CodeScore achieved absolute improvements of 40.56%, 55.07%, and 58.87% on $\tau , r _ { s } ,$ and $r _ { p } ,$ respectively. With an $r _ { s }$ value greater than $0 . 6 ,$ it is evident that there is a strong correlation between CodeScore and Ground Truth. Furthermore, CodeScore has the lowest MAE compared to other EMs. The execution time of CodeScore is similar to other LLM-based EMs and slightly longer than existing Match-based CEMs. However, compared to the 20.7??×, 28.7??×, and 22.1??× execution time of execution-based CEMs in three code evaluation datasets, CodeScore reduces execution time by three orders of magnitude. We also find that computing execution-based CEMs for code evaluation with a small number of original test cases is insufficient. They have a significant reduction in correlation coefficients compared to using larger extended test cases. In cases where test cases are rare or low-quality, such as on APPS-Eval, the correlation between our CodeScore and Ground Truth even far exceeds that of execution-based CEMs.
...
<table><tr><td>Method</td><td>Value</td><td> $\tau \uparrow$ </td><td> $r_s \uparrow$ </td><td> $r_p \uparrow$ </td><td>MAE  $\downarrow$ </td><td>Execution Time  $\downarrow$ </td></tr><tr><td colspan="7">CodeScore (UniCE with  $\mathcal{L}^{Uni}$ )</td></tr><tr><td>UniXcoder</td><td>0.4731</td><td>0.4997</td><td>0.6597</td><td>0.6486</td><td>0.2179</td><td>1.00  $\times$ </td></tr><tr><td>CodeBert</td><td>0.4809</td><td>0.4675</td><td>0.6236</td><td>0.5622</td><td>0.2344</td><td>0.98  $\times$ </td></tr><tr><td>CodeGraphBert</td><td>0.4597</td><td>0.5073</td><td>0.6728</td><td>0.6480</td><td>0.2281</td><td>1.07  $\times$ </td></tr></table>
...
![](images/609038caf5e2ea1b291def5fedeef8b0a1e584c2e10a798400f67c105ea93541.jpg)
...
<table><tr><td>NL</td><td colspan="2">Write a function to find numbers divisible by m and n from a list of numbers using lambda function.</td></tr><tr><td>Reference Code</td><td colspan="2">def div_of_nums(nums,m,n): result = list(filter(lambda x: (x % m == 0 and x % n == 0), nums)) return result</td></tr><tr><td colspan="2">Generated Code I.1 ✗</td><td>Generated Code I.2 √</td></tr><tr><td>def div_of_nums(nums,m,n): ans = [] for n in nums: if n%m == 0: ans.append(n) return ans</td><td>CodeScore: 0.4159 CodeScore (NL-only): 0.4946 CodeScore (Ref-only): 0.3982 CodeBLEU: 0.2283 BERTScore: 0.8862 CodeBERTScore: 0.8615</td><td>def div_of_nums(nums,m,n): return [x for x in nums if x % m == 0 and x % n == 0] CodeScore: 0.9971 CodeScore (NL-only): 0.8323 CodeScore (Ref-only): 0.9631 CodeBLEU: 0.2456 BERTScore: 0.9005 CodeBERTScore: 0.8484</td></tr></table>

(a) Case I

<table><tr><td>NL</td><td colspan="3">Write a python function to check whether the given two numbers have same number of digits or not.</td></tr><tr><td>Reference Code</td><td colspan="3">def same_Length(A,B):while (A &gt; 0 and B &gt; 0):A = A / 10;B = B / 10;if (A == 0 and B == 0):return True;return False;</td></tr><tr><td colspan="2">Generated Code II.1 ✗</td><td colspan="2">Generated Code II.2 ✗</td></tr><tr><td>def same_Length(iterable, n):p = (n-1) *iterablereturn p</td><td>CodeScore: 0.1272CodeScore (NL-only): 0.0962CodeScore (Ref-only): 0.1540CodeBLEU: 0.1383BERTScore: 0.8542CodeBERTScore: 0.7021</td><td>def same_Length(A,B):if len(str(A))==len(str(B)):return Trueelse:try: float(str(str( B)))except ValueError:return Falseelse:return False</td><td>CodeScore: 0.5712CodeScore (NL-only): 0.6442CodeScore (Ref-only): 0.5419CodeBLEU: 0.1744BERTScore: 0.8454CodeBERTScore: 0.7816</td></tr></table>

(b) Case II

<table><tr><td>NL</td><td colspan="3">Write a function to return true if the given number is even else return false.</td></tr><tr><td>Reference Code</td><td colspan="3">def even_num(x):if x%2==0:return Trueelse:return False</td></tr><tr><td colspan="2">Generated Code III.1 ✗</td><td colspan="2">Generated Code III.2 √</td></tr><tr><td>def even_num(n,count = 0):if (n == 0):return Falseif (count % 2 == 101):return (n % 2 == 3)else:return (2 * n - 1) + (n % 4 - 1) &gt; 1</td><td>CodeScore: 0.3590CodeScore (NL-only): 0.4350CodeScore (Ref-only): 0.2988CodeBLEU: 0.3582BERTScore: 0.8882CodeBERTScore: 0.7764</td><td>def even_num(x):return True if x % 2 == 0else False</td><td>CodeScore: 0.9904CodeScore (NL-only): 1.0000CodeScore (Ref-only): 0.9710CodeBLEU: 0.3421BERTScore: 0.8665CodeBERTScore: 0.8345</td></tr></table>
...
<table><tr><td>NL</td><td colspan="2">You&#x27;ll be given a string of words, and your task is to count the numberof boredoms. A boredom is a sentence that starts with the word &quot;l&quot;. Sentences are delimited by &#x27;.&#x27;, &#x27;?&#x27; or &#x27;l&#x27;.</td></tr><tr><td>Reference Code</td><td colspan="2">def is_bored(S):
    import re sentences = re.split(r&#x27;[.?!]\s*&#x27;, S)
    return sum(sentence[0:2] == &#x27;I&#x27; for sentence in sentences)</td></tr><tr><td colspan="2">Generated Code 1 ✗</td><td>Generated Code 2 √</td></tr><tr><td colspan="2">def is_bored(S):
    return [&#x27;I&#x27; in i for i in S.split(&#x27; &#x27;) 
Exec: 0</td><td>def is_bored(S):
    return len(filter(lambda x: x[0] == &#x27;I&#x27;, S.split())) 
Exec: 1</td></tr></table>
...
CodeScore: Evaluating Code Generation by Learning Code ExecutionConference acronym ’XX, June 03–05, 2018, Woodstock, NY
...
![](images/2214250e2c9a47119f92fa94ed4fad491c0a6ebce469825395f95c171f4a0e61.jpg)  
...
![](images/3c40b5f18942524b4342049adb53838c4928e601a805170b24987118d06d7040.jpg)
