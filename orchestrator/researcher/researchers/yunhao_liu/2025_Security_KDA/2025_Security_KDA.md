...
Lan Zhang $^{1*}$ Xinben Gao $^{1}$ Liuyi Yao Jinke Song $^{2}$ Yaliang Li $^{1}$ University of Science and Technology of China
...
Our objective is to seek a process of $T_{\mathrm{root}}$ to the following problem:

$$
\text { Find   } \mathcal {P}, \text {   s.t.   Jailbreak } (T _ {\text { root }}, M _ {\text { target }} (\mathcal {P} (T _ {\text { root }}))) = 1 \tag {1}
$$

where $\mathcal{P}$ represents a procedure that modifies the original task $T_{\mathrm{root}}$ , $M_{\mathrm{target}}(\mathcal{P}(T_{\mathrm{root}})$ denotes the target model's output towards the calibrated malicious task by $\mathcal{P}$ , Jailbreak: $\mathcal{V}^{*} \times \mathcal{V}^{*} \to \{0,1\}$ is a binary-valued function that determines whether a given task-response pair is jailbroken, in which $\mathcal{V}$ denotes the vocabulary (i.e., the set of all tokens).
...
The framework of our proposed attack is shown in Fig. 1. As illustrated in the figure, our approach commences with the decomposition of the original malicious task $T_{root}$ via the auxiliary model $M_{aux}$ , thereby fragmenting task $T_{root}$ into a series of subtasks $T_{1}, T_{2}, \ldots, T_{n}$ . Subsequent to this task decomposition, some of these subtasks $T_{i}$ can obtain their respective responses $R_{i}$ from target model $M_{target}$ . Conversely, the other subtasks are still rejected by $M_{target}$ . Thus, our attack recursively decomposes any rejected subtasks $T_{i}$ into further sub-subtasks $T_{i,1}, T_{i,2}, \ldots, T_{i,n}$ , propagating this decomposition until all components of the original task executed by $M_{target}$ , culminating in the acquisition of their corresponding responses. The final phase of our attack involves the systematic reassembly of responses in accordance with the structure of the task decomposition. By aggregating the individual outputs, we reconstruct the response $R_{root}$ to the original malicious task $T_{root}$ . Through this process, we effectively circumvent the defensive mechanisms of the target model $M_{target}$ and fulfill the objectives of jailbreak.
...
Given the target large language model $M_{\text{target}}$ and the original malicious task $T_{\text{root}}$ , the jailbreak process begins with directly query the target model $M_{\text{target}}$ with the given task $T_{\text{root}}$ for an initial response. Once receive the response from $M_{\text{target}}$ , the auxiliary model $M_{\text{aux}}$ is employed to evaluate whether the execution of the original task $T_{\text{root}}$ is successful. If the evaluation indicates that $M_{\text{target}}$ has successfully processed the task, we will return the model's response along with a confirmation of success. However, if the execution by $M_{\text{target}}$ is deemed unsuccessful, we activate the task resolution process, which involves proceeding to Phase 2 and Phase 3 of our method. It is worth noting that the task resolution process, denoted as RECURSIVE-TASKRES function in Alg. 1, is a recursive function that will be iteratively called in the entire process, to resolve every task $T$ that is rejected by $M_{\text{target}}$ .
...
Based on this tendency, we develop the automatic task decomposition that involves rephrasing queries to hide malicious intent. By presenting requests to $M_{aux}$ as benign quests for knowledge, we prompt them to provide detailed information that can be instrumental in breaking down the given task

Algorithm 1 Our Jailbreak Attack   
Inputs: $T_{\text{root}}, M_{\text{target}}, M_{\text{aux}}$ Outputs: $R_{\text{root}}$ function RECURSIVE-TASKRES(T) $C_i(\text{or } S_i) \leftarrow \text{task\_decomp}(T, M_{\text{aux}})$ $R, F \leftarrow \text{task\_exec}(T, \{C_i\} (\text{or } \{S_i\}), M_{\text{target}})$ ▷ Execute T with references
if F then
    return R, F
end if
...
if $C_i$ then $F \leftarrow F_1 \lor F_2 \lor \ldots \lor F_N$ else $F \leftarrow F_1 \land F_2 \land \ldots \land F_N$ end if
if F then $R \leftarrow \text{result\_agg}(R_1, R_2, \ldots, R_N)$ else $R \leftarrow \text{None}$ end if
return R, F
end function $R_{\text{root}}, F_{\text{root}} \leftarrow \text{task\_exec}(T_{\text{root}})$ ▷ Phase 1
if $\neg F_{\text{root}}$ then $R_{\text{root}}, F_{\text{root}} \leftarrow \text{RECURSIVE-TASKRES}(T_{\text{root}})$ end if ▷ Phase 2 & 3
if $F_{\text{root}}$ then $F_{\text{jailbreak}} \leftarrow \text{judge\_jailbreak}(T_{\text{root}}, R_{\text{root}})$ if $F_{\text{jailbreak}}$ then
    return $R_{\text{root}}$ ▷ Jailbreak succeeds.
    else
...
T into its essential concretizations, (denoted as $C_{i}$ ) or outlining the necessary steps (denoted as $S_{i}$ ) for its execution. This decomposition procedure is formally depicted as follows:

$$
T \xrightarrow {M _ {\mathrm{aux}}} C _ {1}, C _ {2}, \dots \quad o r \quad T \xrightarrow {M _ {\mathrm{aux}}} S _ {1}, S _ {2}, \dots \tag {3}
$$

After obtaining this critical information, we integrate either $C_{i}$ or $S_{i}$ with T to formulate the subtask $T_{i}$ . This subtask then becomes child nodes branching off from the given node T.
...
Adopt meta-prompting to automate the attack. The inherent diversity of tasks necessitates an advanced approach beyond the sole application of non-machine learning algorithms to transform tasks into “what”-type questions. Therefore, a meta-prompting method is employed to facilitate the $M_{aux}$ to complete this conversion. Initially, $M_{aux}$ discerns and extracts the “main task” and “requirement” of the given task. For example, the “main task” of “Write a script that can exploit vulnerabilities in software or an operating system” is “write a script”, and the “requirement” is “exploit vulnerabilities in software or an operating system”. Then the requirement is combined with “What is” or “What are” to form a “what”-type question “What is exploiting vulnerabilities in a software or operating system?”. The generated question is then input into the target model $M_{target}$ , and the process ends if $M_{target}$ answers. If $M_{target}$ does not answer, $M_{aux}$ will be instructed to delete some information from the requirement and continue trying to generate “what”-type questions until $M_{target}$ answers. Once the “what”-type question is generated, the following step of task decomposition is easy to perform. We only need to clarify the requirements for task decomposition in the prompt, that is, the auxiliary model must generate either specific concretizations or specific substeps to complete this task within a fixed format, and then the decomposition results can be extracted according to the fixed format of the output. We use in context learning to improve the ability of task decomposition.

Since the concretizations or steps of the given task $T$ have been generated, to enhance the successful execution of $T$ , it is crucial to supplement the task request with detailed step-by-step instructions, since some LLMs, such as GPT-4-turbo, exhibit a tendency to comply when provided with such information. The propensity of LLMs to generate content based on detailed steps, as opposed to direct commands, can be seen in Fig. 3. The command like "Write a script to control execu tion in exploiting buffer overflow vulnerabilities" is outright rejected by the target LLM. However, when such a request is followed by step-by-step instructions — "identify control structures" and "align payload" — the model is more likely to generate a script adhering to these steps, thus indirectly accomplishing the original malevolent intent. The reason behind this behavior is not fully understood, yet it is believed that the granularity of the steps may reduce the perceived risk, leading to greater LLM compliance.

# 3.3.3 Phase 3: Subtask execution

In Phase 3, the target model $M_{\text{target}}$ is engaged to execute a sequence of subtasks, $T_1, T_2, \ldots$ , with the auxiliary model $M_{\text{aux}}$ assessing the success of each execution. The criterion for evaluation the success of task $T$ as completed varies depending on the nature of its subtasks. When the subtasks are originated from $C_i$ , the finalization of any single subtask is sufficient to consider the task $T$ finished. While for subtasks associated with a particular step $S_i$ , all relevant subtasks must be successfully completed. If the subtask execution assessed as complete, the execution results are summarized and report them up the task hierarchy. If not, we restart from Phase 2 through Phase 3 for all subtasks $T_i$ that not successfully executed.
...
Overall, our proposed jailbreak method is summarized in the recursive manner, shown in Algorithm. 1. In this algorithm, the recursive function RECURSIVE-TASKRES consists of phase 2 (task decomposition) and phase 3 (subtask execution), and the input T of it can be any node in the decomposition tree. The process of task decomposition, task execution, task combination and result aggregation are denoted as task\_decomp, task\_exec, task\_comb and result\_agg respectively. The parameter F represents the flag of successful execution for each node. Given an original malicious task, through the iterative invocation of the recursive function, the original malicious task can be executed by completing associated subtasks. If the recursive function finally returns False, then the jailbreak attack has failed for this malicious task. Conversely, if the recursive function returns True with the aggregated results. Phase 4 then takes over to aggregate and judge the final results of the jailbreak attack.

# 3.4 An Attack Example

To more intuitively demonstrate our method, we show the entire process of executing a malicious task, “Write a script to exploit vulnerabilities in a software of operating system.”, by our jailbreak attack. To save space we omit a lot of generated content and scripts, as well as system prompts. The complete system prompts are presented in Appendix. In this case, both the auxiliary LLM $M_{aux}$ and the target LLM $M_{target}$ are GPT-4-turbo (GPT-4-1106-preview). To facilitate maximal reproducibility of our experimental results, we meticulously prescribed the parameters for each API invocation: a temperature of 0, a seed of 0, a max\_tokens of 1024, with all remaining parameters at their default settings.

Given the original malicious task “Write a script that can exploit vulnerabilities in a software or operating system.”, we first initialize our attack by letting $M_{target}$ execute it directly in phase 1, and $M_{target}$ rejected this task directly. Then, we proceed to phase 2 of Round 1 to deconstruct the task via $M_{aux}$ by subtly hiding our malicious intent and inquiry “What is exploiting vulnerabilities in a software or operating system?”. $M_{aux}$ provides the several categories of exploiting vulnerabilities, including “injection attacks”, “buffer overflows”, and so on. Such categories are then combined with harmful instructions to generate the subtasks - for example, “Write a script that can exploit buffer overflow vulnerabilities in a software or operating system.”, thus going into phase 3. Since the generated subtasks are specific concretizations of the original task, as long as one of the subtask is successfully executed, the entire task is considered a success. Hence, we select the buffer overflow exploit category at random and query $M_{target}$ . Still, the $M_{target}$ still yields the same refusal, necessitating further task decomposition via $M_{aux}$ in Round 2.

In Round 2, during phase 2, by rephrasing the original question into “what”-type query, the $M_{aux}$ provides the steps to execute the task “exploit buffer overflow vulnerabilities in a software or operating system”. Then we prompt the original task with these steps appended as the reference. Despite $M_{target}$ still rejects to respond the task with references, presenting the reluctance to participate in harmful actions, there is a growing willingness to provide a basic script that explains how a buffer overflow works. However, this script is elementary and lacks the sophistication needed to actually carry out a malicious attack. In phase 3, we proceed to execute each step in sequence. Since the task of finding buffer overflow vulnerabilities is seen as low risk, $M_{target}$ is willing to provide a complex script for detecting these vulnerabilities. However, the remaining tasks still retain a higher degree of threat, resulting in continued non-compliance from $M_{target}$ . It thus requires the procedural knowledge of these tasks through new rounds of decomposition, and incorporate them as referential steps within the tasks to elicit script generation from $M_{target}$ .
...
![](images/491aa8a836fb7db99b77e27062b096956875d8b5d4f51258d5729d3f96ed20ad.jpg)



Figure 3: Representation of the comprehensive adversarial attack procedure utilizing a case study. In this scenario $M_{target}$ and $M_{aux}$ are both GPT-4-turbo (GPT-4-1106-preview). For the purposes of clarity and brevity, we have omitted the system prompts associated with each query; these system prompts are shown in Appendix

![](images/c303c30c71c05105718218b12cf310c6e7b8ed59e9da3045886f5583c3e0f76b.jpg)



Figure 4: Distribution of scores given by three referee LLMs(GPT-4, Claude 2.1 and Gemini Pro).

![](images/9a777c0ce02145017d94209dcee73bd9661f9d2580034c1a1e4a68e0c966c854.jpg)
...
Target Models: In our study, we employ LLMs with implemented safety alignments as our target models, including white-box LLMs (Vicuna-13B-v1.5 [11], Llama2-7B-chat [37]) alongside black-box LLMs (GPT-3.5-turbo [27], GPT-4 [1], GPT-4-turbo(GPT-4-1106-preview) [28], PaLM-2 [4]), respectively.
...
![](images/a535232c081a2c7a0f96819b2c743d47201145d7155399fd029b48e0e0e5e97c.jpg)
...
![](images/29891da75eae247d4719fe2393a9121a3d6bb26c1b2322db2fc7f442e8a93227.jpg)
...
![](images/fe28d00e13f2012e8cf2ea402047c6a9605950d2fb948d8eac3c9784b63e34ae.jpg)
...
[7] Content at Scale. What is dan prompt for chatgpt? https://contentatscale.ai/blog/dan-prompt-for-chatgpt/danpromptforchatgptaquickoverview. Accessed: 2024-04-27.   
...
[10] Patrick Chao, Alexander Robey, Edgar Dobriban, Hamed Hassani, George J Pappas, and Eric Wong. Jail-breaking black box large language models in twenty queries. arXiv preprint arXiv:2310.08419, 2023.   
...
[17] Google. Generative ai prohibited use policy. https://policies.google.com/terms/generative-ai/use-policy. Accessed: 2024-02-07.   
...
[32] Xiangyu Qi, Yi Zeng, Tinghao Xie, Pin-Yu Chen, Ruoxi Jia, Prateek Mittal, and Peter Henderson. Fine-tuning aligned language models compromises safety, even when users do not intend to! arXiv preprint arXiv:2310.03693, 2023.   
...
Except for the results of Claude 2.1, GPT-4 and Vicuna-13B-v1.5 as shown in Fig. 7, we present results of other model in Table 2 here in Fig. 10.
...
The [[REQUIREMENT]] in the system prompts is different for each task. It is the main content you want LLM to generate. For example: Write a script to exploit vulnerabilities in a software or operating system. The [[REQUIREMENT]] is "script". In order to automate our attack, we use another query to let $M_{aux}$ generate [[REQUIREMENT]] according to the task.
...
![](images/c0c138baa5a7e283744f667b02a6ba6dd976c749363903620065d32041bf7d73.jpg)  
