...
- `Scoring, Reasoning, and Selecting the Best! Ensembling Large Language Models via a Peer-Review Process`: 引用它来支持 LLM ensemble 应该像同行评审一样评分、推理、选择。
- `Scoring, Reasoning, and Selecting the Best! Ensembling Large Language Models via a Peer-Review Process`: 引用它来支持 fully unsupervised、LLM-as-judge、Dawid-Skene 加权聚合。
- `Scoring, Reasoning, and Selecting the Best! Ensembling Large Language Models via a Peer-Review Process`: 引用它来反对朴素投票、无去偏机制、O(J!) 式昂贵组合。
- `LLMBoost_ Make Large Language Models Stronger with Boosting`: 引用它来支持 break the black-box barrier，并让后继模型访问前驱中间隐状态。
- `LLMBoost_ Make Large Language Models Stronger with Boosting`: 引用它来支持 cross-model attention、state-driven hierarchical interaction、hidden-representation fusion。
- `LLMBoost_ Make Large Language Models Stronger with Boosting`: 引用它来支持 key-token rectification、incorrect-logit suppression、单模型级效率约束。
