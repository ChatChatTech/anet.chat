...
- `AdaFit_ Rethinking Learning-based Normal Estimation on Point Clouds`: 引用它来说明预测额外 point-wise offset 可同时缓解 polynomial order mismatch 与 outlier sensitivity，并证明 denoising、weight resetting、truncating 都不足以替代几何位置校正。

## WHITEBOARD BEHAVIOR (anet.chat 特定)

- 用短句发 sticky-note，每条不超过 2 句。
